...
import { scrollEffect,scrollFunctionTriggerD,hiddenEffect,navbarUpeffect } from "./modules/observer-api.js";
import { scrollMeffect } from "./modules/scrollmagic.js";
...
const AddclassActive = (entrada) => {   
        entrada.firstChild.classList.add("reveal-t-p")
};
...
d.addEventListener("DOMContentLoaded",(e)=>{
    ...
    scrollMeffect()
    mouseOver()
    navbarUpeffect(".nav-container",".title-project","title-project-e")
    scrollEffect(".wrapper-text-ww",".wrapper-text-ww","max-text-ww")
    scrollFunctionTriggerD(AddclassActive,".wrapper-text-effect")
    hiddenEffect("h1[id]","wrapper-comments-title")
    setTimeout(() => {
      d.querySelectorAll(".img-show").forEach(element => {
        element.classList.remove("img-show-f")
      });
    }, 1400);
})
...
d.addEventListener("click", e =>{
    ...
    if (e.target.matches(".wrapper-explorer-arrow a")) {
        e.preventDefault()
    }
